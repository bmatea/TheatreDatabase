...
{
    public class Purchase : Actions
    {
        public override decimal PerformAction(decimal x, int acc, int audit)
        {
            String connectionStr = @"Server =.\SQLEXPRESS; Database = TheatreDatabase; Trusted_Connection = True;
                                   MultipleActiveResultSets = True";


            SqlConnection connection = new SqlConnection(connectionStr);
            connection.Open();
            SqlCommand command = new SqlCommand("PurchaseTicket", connection);
